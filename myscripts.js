...
// 🔥 السحر هنا: استدعاء منبه الأندرويد عبر JSON
        if (reminderTimestamp) {
            try {
                // التحقق من وجود الجسر بشكل صحيح
                if (typeof AndroidBridge !== "undefined") {
                    // إرسال المتغيرات مباشرة للأندرويد
                    AndroidBridge.setAlarmDirect(
                        docRef.id, 
                        reminderTimestamp, 
                        name, 
                        product, 
                    ...
                    // سيظهر هذا التنبيه إذا كنت تفتح التطبيق من جوجل كروم بدلاً من التطبيق نفسه
                    console.log("⚠️ التطبيق غير متصل بنظام الأندرويد. التنبيه سيعمل فقط أثناء فتح الشاشة.");
                }
            } catch (err) {
                console.error("Bridge Error:", err);
                Swal.fire('خطأ تقني', 'فشل ربط المنبه بنظام الهاتف: ' + err.message, 'error');
            }
        }
...
// 4. دالة عرض السجل (Render History) - محدثة للسيرفر
...
window.renderHistory = function() {
    ...
    // إيقاف الاستماع القديم لتجنب التكرار
        ...
        .onSnapshot((snapshot) => {
            ...
            tbody.innerHTML = snapshot.docs.map(doc => {
                ...
                <tr>
                    ...
                    <td style="padding:10px; border-bottom:1px solid #eee;">
                        <button onclick="deleteCloudRecord('${doc.id}')" style="color:red; border:none; background:none; cursor:pointer; font-size:16px;">❌</button>
                    </td>
                </tr>`;
            }).join('');
        }, (error) => {
